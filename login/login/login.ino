...
void cambiarClave() {
  Serial.println(" ");
  Serial.println("=== ESPERANDO CONTRASENIA =="); //estado
  ...
  char nuevaClave[9]; // variable temporal para almacenar la nueva clave
  ...
  while (true) {
    ...
    if (TECLA) {
      if (TECLA == '#') {
        ...
        Serial.println("Nueva contrasenia almacenada."); //estado
        Serial.println(" ");
        modoSeguridad();
        delay(300);
  ...
  while (true) {
    ...
    if (TECLA == '#') {
      ...
      if (strcmp(CLAVE, CLAVE_DEFECTO) == 0) {
        ...
        modoSeguridad(); // entra al modo seguridad
      ...
      CLAVE[INDICE] = TECLA;
      INDICE++;
      Serial.print("*"); //imprime * en lugar de la contrasenia
    } 
  }
}
